...
public class SessionServices : ISessionServices
{
        ...
        if (_contextAccessor.HttpContext != null)
        {
            ...
            if (loginTokens != null)
            {
                ...
                _logger.LogInformation(
                    $"[User Login {DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss")}] User Id {id} logged in." +
    ...
    public string GetTokenByKey(int id)
    {
        if (_contextAccessor.HttpContext != null)
        {
            ...
            if (json != null) loginTokens = JsonConvert.DeserializeObject<Dictionary<int, string>>(json);

            _logger.LogInformation(
                $"[Verify Token {DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss")}] Current User Id is {id}.");

            if (loginTokens != null && loginTokens.ContainsKey(id)) return loginTokens[id];
        }
    ...
    public bool RemoveToken(int id)
    {
        if (_contextAccessor.HttpContext != null)
        {
            ...
            _logger.LogInformation(
                $"[User Logout {DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss")}] Current User Id is {id}.");
            ...
            if (loginTokens != null && loginTokens.ContainsKey(id))
            {
                ...
                //Console.ForegroundColor = ConsoleColor.White;
                _logger.LogInformation(
                    $"[User Logout {DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss")}] User Id {id} logged out." +
